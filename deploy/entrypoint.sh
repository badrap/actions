...
sops -d "${INPUT_SECRETS_PATH}" | helm upgrade "${INPUT_HELM_RELEASE}" helm-chart --install --atomic --namespace "${INPUT_HELM_NAMESPACE}" --create-namespace --values - --set image.repository="${INPUT_GCLOUD_CONTAINER_REPOSITORY}" --set image.tag="${GITHUB_SHA}"
